...
(function() {
  ...
  config.$inject =  ['$stateProvider', '$urlRouterProvider', '$httpProvider', '$provide', 'MODULE_CONFIG','$authProvider'];

  function config( $stateProvider, $urlRouterProvider, $httpProvider, $provide, MODULE_CONFIG, $authProvider ) {
    ...
    function redirectToAuth($q, $injector) {
    ...
    $provide.factory('redirectToAuth', redirectToAuth);

    $httpProvider.interceptors.push('redirectToAuth');

    $authProvider.loginUrl = 'http://localhost:8000/login';

    $urlRouterProvider
        .otherwise('../views/misc/signup.html');
    $stateProvider
        .state('app', {
          abstract: true,
          url: '/app',
          views: {
            '': {
              ...
              resolve: load('scripts/controllers/sidebarController.js')
            }
          }
        })
        .state('app.dashboard', {
